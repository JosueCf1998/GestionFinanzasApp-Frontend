...
interface TransactionListItem extends CompactListItem {
  id: number;
}
...
export class DetailTransactionPage implements OnInit, OnDestroy {
  ...
  trackByTransaction(_: number, item: TransactionListItem): number {
    return item.id;
  }
  ...
  private toListItem(transaction: FilteredTransaction): TransactionListItem {
    return {
      id: transaction.id,
